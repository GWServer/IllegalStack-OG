...
public class Listener114 implements Listener {
    ...
    public Listener114(IllegalStack illegalStack) {
        ...
        Material[] consume = new Material[] { Material.APPLE, Material.BAKED_POTATO, Material.BEETROOT,
                Material.BEETROOT_SOUP, Material.BREAD, Material.CARROT, Material.CHORUS_FRUIT, Material.COOKED_CHICKEN,
                Material.COOKED_COD, Material.COOKED_MUTTON, Material.COOKED_PORKCHOP, Material.COOKED_SALMON,
                Material.COOKED_RABBIT, Material.COOKIE, Material.DRIED_KELP, Material.GOLDEN_APPLE,
                Material.ENCHANTED_GOLDEN_APPLE, Material.GOLDEN_CARROT, Material.MELON_SLICE, Material.MUSHROOM_STEW,
                Material.POISONOUS_POTATO, Material.POTATO, Material.PUFFERFISH, Material.PUMPKIN_PIE,
                Material.RABBIT_STEW, Material.BEEF, Material.CHICKEN, Material.COD, Material.MUTTON, Material.PORKCHOP,
                Material.RABBIT, Material.SALMON, Material.ROTTEN_FLESH, Material.SPIDER_EYE, Material.COOKED_BEEF,
                Material.SUSPICIOUS_STEW, Material.SWEET_BERRIES, Material.TROPICAL_FISH, Material.MILK_BUCKET,
                Material.POTION };
    ...
    public void OnVillagerTransform(EntityTransformEvent e) {
        ...
        if (Protections.ZombieVillagerTransformChance.getIntValue() < 100
                && !Protections.ZombieVillagerTransformChance.isDisabledInWorld(e.getEntity().getWorld()))
        {
    ...
    public void OnVillagerInteract(PlayerInteractEntityEvent e) {
        ...
        if (Protections.PreventVillagerSwimExploit.isEnabled(e.getPlayer())) {
            ...
            if (e.getRightClicked() != null && e.getRightClicked() instanceof Villager
                    && e.getRightClicked() instanceof Merchant)
            {
                ...
                if (is.getType() == Material.ENCHANTED_BOOK) {
                    ...
                    for (Enchantment enchantment : enchantmentStorageMeta.getStoredEnchants().keySet()) {
                        ...
                        if (enchantment.equals(Enchantment.SILK_TOUCH)) {
                            ...
                            e.setCancelled(true);
                            fListener.getLog()
                                    .append2(Msg.SilkTouchBookBlocked.getValue(p, e.getBlock().getType().name()));
    ...
    public void PreventVillagerRestock(VillagerReplenishTradeEvent e) {
        ...
        if (System.currentTimeMillis() >= lastTrade.get(e.getEntity().getUniqueId())) {
            ...
            lastTrade.put(e.getEntity().getUniqueId(),
    ...
    public void onHandSwap(PlayerSwapHandItemsEvent e) {
        ...
        if (consumables.contains(item1.getType()) && consumables.contains(item2.getType())
        ...
        if (Protections.PreventBedrockDestruction.isEnabled(event.getWorld())) {
            ...
            for (BlockState b : event.getBlocks()) {
                ...
                if (b.getY() > b.getWorld().getMaxHeight()) {
                    ...
                    // Blocking portals spawning at world height limit, preventing from
    ...
    public void onPearlHit(ProjectileHitEvent e) {
        ...
        if (Protections.PreventPearlGlassPhasing.isEnabled(e.getEntity().getLocation())) {
            ...
            if (!(e.getEntity() instanceof EnderPearl) || e.getHitBlock() == null
                    || !(e.getEntity().getShooter() instanceof Player) || e.getHitBlockFace() != BlockFace.UP)
            {
    ...
    public void onItemSwitch(PlayerItemHeldEvent e) {
                ...
                && item1.getType() == item2.getType())
        {
            ...
            Scheduler.runTaskLater(IllegalStack.getPlugin(), () -> e.getPlayer().getInventory().setItem(slot, food), 3,
                    e.getPlayer());
    ...
    public void onFallingBlockSpawn(EntitySpawnEvent e) {
        ...
        if (e.getEntity() instanceof Projectile && !(e.getEntity() instanceof FishHook)
                && Protections.PreventProjectileExploit2.isEnabled(e.getEntity().getLocation()))
        {
        ...
        if (!(e.getEntity() instanceof FallingBlock)
                || !Protections.PreventVibratingBlocks.isEnabled(e.getLocation()))
        {
