...
public class Listener114 implements Listener {
    ...
    public void OnVillagerTransform(EntityTransformEvent e) {
        ...
        if (Protections.ZombieVillagerTransformChance.getIntValue() < 100
                && !Protections.ZombieVillagerTransformChance.isDisabledInWorld(
            ...
            if (p != null && p.getInventory().getItemInMainHand() != null) {
                ...
                if (is.getType() == Material.ENCHANTED_BOOK) {
                    ...
                    for (Enchantment enchantment :
                            enchantmentStorageMeta.getStoredEnchants().keySet()) {
                        if (enchantment.equals(Enchantment.SILK_TOUCH)) {
                            ...
                            fListener
                                    .getLog()
                                    .append2(Msg.SilkTouchBookBlocked.getValue(
                                            p, e.getBlock().getType().name()));
                        }
                    }
                }
            }
    ...
    public void LockMerchantTrades(InventoryOpenEvent e) {
        ...
        if (Protections.PreventCommandsInBed.isEnabled(e.getPlayer())
    ...
    public void onHandSwap(PlayerSwapHandItemsEvent e) {
        ...
        if (consumables.contains(item1.getType())
                && consumables.contains(item2.getType())
        ...
        if (Protections.PreventBedrockDestruction.isEnabled(event.getWorld())) {
            ...
            for (BlockState b : event.getBlocks()) {
                if (fListener.getUnbreakable().contains(b.getType())) {
                    ...
                    LOGGER.info(
                            "Portal tried to break an unbreakable block: {}",
                            b.getType().name());
    ...
    public void onPearlHit(ProjectileHitEvent e) {
        ...
        if (Protections.PreventPearlGlassPhasing.isEnabled(e.getEntity().getLocation())) {
            if (!(e.getEntity() instanceof EnderPearl)
                    ...
                    || e.getHitBlockFace() != BlockFace.UP) {
                return;
            }
            for (BlockFace face : faces) {
                ...
                if (!fListener.getAirBlocks().contains(above.getType())) {
                    ...
                    Location l = e.getHitBlock()
                            .getRelative(e.getHitBlockFace())
                            .getLocation()
                            .add(0.5, 0.5, 0.5);
    ...
    public void onItemSwitch(PlayerItemHeldEvent e) {
        ...
        if (consumables.contains(item1.getType())
            ...
            Scheduler.runTaskLater(
                    IllegalStack.getPlugin(), () -> e.getPlayer().getInventory().setItem(slot, food), 3, e.getPlayer());
        }
    }
