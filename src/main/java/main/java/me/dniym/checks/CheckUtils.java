    ...
    public static boolean CheckEntireContainer(Container container) {
        ...
        for (ItemStack itemStack : container.getInventory()) {
            ...
            if (itemStack != null && IllegalStack.hasShulkers() && itemStack.getType().name().contains("SHULKER_BOX")) {
                ...
                if (tagSize > 0 && IllegalStackAction.isCompleted(Protections.DestroyInvalidShulkers, itemStack,
                        container.getInventory()))
                {

                    fListener.getLog().append(Msg.StaffBadShulkerRemoved.getValue(container.getLocation(), tagSize),
                    ...
                    if (itemStack.getItemMeta() instanceof BlockStateMeta) {
                        ...
                        if (im.getBlockState() instanceof ShulkerBox) {
                            ...
                            Inventory inv = Bukkit.createInventory(null, shulker.getInventory().getSize(),
                                    "IScontainerCheck");
                            for (ItemStack itemStack1 : shulker.getInventory().getContents()) {
                                ...
                                if (tagSize > 0 && IllegalStackAction.isCompleted(Protections.DestroyInvalidShulkers,
                                        itemStack1, shulker.getInventory()))
                                {
