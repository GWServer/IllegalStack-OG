...
public class TrackedProjectile {
    ...
    public TrackedProjectile(Projectile proj) {
        ...
        if (alreadyTracked(proj) || !shouldTrack(proj)) return;
    ...
    private boolean shouldTrack(Projectile proj) {
        if (proj != null) {
            if (proj instanceof Arrow || proj instanceof ThrowableProjectile) return true;
        }
        return false;
    }
    ...
    public static boolean alreadyTracked(Entity ent) {
        ...
        for (TrackedProjectile tp : trackedProjectiles) if (ent.equals(tp.entity)) return true;
