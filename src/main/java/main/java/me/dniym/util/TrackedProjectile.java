...
public class TrackedProjectile {
    ...
    /*
     * I cannot figure out why this will not work async. Doing this async always
     * seems to result in the server crashing.. I suspected the problem was due to
     * the ProjectileLaunchEvent is fired twice whenever a projectile is launched,
     * this seemed to be causing 4 objects to get tracked when 2 were fired..
     * However the crash persists whenever I try to do this async it pukes. Nothing
     * that is going on here is difficult to do sync. I have moved this to a sync
     * timer and it works flawlessly...
     */
    public static void manageAsync() {
        ...
        // IllegalStack.getLogger("There are " + trackedProjectiles.size() + "
        // projectiles being tracked");
        trackedProjectiles.removeAll(removed);
